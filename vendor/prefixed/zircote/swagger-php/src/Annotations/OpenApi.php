...
class OpenApi extends AbstractAnnotation
{
    ...
    public const SUPPORTED_VERSIONS = [self::VERSION_3_0_0, '3.0.1', '3.0.2', '3.0.3', '3.0.4', self::VERSION_3_1_0, '3.1.1'];
    /**
     ...
     * @inheritdoc
     */
    public static $_nested = [Info::class => 'info', Server::class => ['servers'], PathItem::class => ['paths', 'path'], Components::class => 'components', Tag::class => ['tags'], ExternalDocumentation::class => 'externalDocs', Webhook::class => ['webhooks', 'webhook'], Attachable::class => ['attachables']];
    /**
     * @inheritdoc
     */
    ...
    public function validate(?array $stack = null, ?array $skip = null, string $ref = '', $context = null) : bool
    {
        ...
        if (self::versionMatch($this->openapi, '3.1.x') && Generator::isDefault($this->paths) && Generator::isDefault($this->webhooks) && Generator::isDefault($this->components)) {
    ...
    public static function versionMatch(string $version1, string $version2) : bool
    {
        $expand = function (string $v) : array {
            if (substr_compare($v, '.x', -strlen('.x')) !== 0) {
                return [$v];
            }
            $minor = str_replace('.x', '', $v);
            return array_filter(self::SUPPORTED_VERSIONS, function (string $sv) use ($minor) : bool {
                return strncmp($sv, $minor, strlen($minor)) === 0;
            });
        };
