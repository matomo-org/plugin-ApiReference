    ...
    protected function expandContextEnum(Analysis $analysis) : void
    {
        ...
        foreach ($schemas as $schema) {
            if ($schema->_context->is('enum')) {
                ...
                $schema->enum = array_map(function (\ReflectionEnumUnitCase $case) use ($useName) {
                ...
                if ($this->enumNames !== null && !$useName) {
                    ...
                    $schemaX[$this->enumNames] = array_map(function (\ReflectionEnumUnitCase $case) : string {
                        return $case->name;
                    }, $re->getCases());
                    $schema->x = $schemaX;
                }
    ...
    protected function expandSchemaEnum(Analysis $analysis) : void
    {
        ...
        foreach ($schemas as $schema) {
            ...
            if (is_string($schema->enum)) {
                ...
                if (is_a($schema->enum, \UnitEnum::class, \true)) {
                ...
                foreach ($schema->enum as $enum) {
                    if (is_string($enum) && function_exists('enum_exists') && class_exists($enum)) {
