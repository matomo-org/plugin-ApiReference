...
class Generator
{
    ...
    public function getAnalyser() : AnalyserInterface
    {
        ...
        $this->analyser = $this->analyser ?: new ReflectionAnalyser([new AttributeAnnotationFactory($generatorConfig['ignoreOtherAttributes']), new DocBlockAnnotationFactory()]);
    ...
    {
        return ['generator' => ['ignoreOtherAttributes' => \false], 'operationId' => ['hash' => \true]];
    }
    ...
    {
        // merge with defaults
        $config = $options + ['aliases' => self::DEFAULT_ALIASES, 'namespaces' => self::DEFAULT_NAMESPACES, 'analyser' => null, 'analysis' => null, 'processor' => null, 'processors' => null, 'config' => [], 'logger' => null, 'validate' => \true, 'version' => null];
        $processorPipeline = $config['processor'] ?? ($config['processors'] ? new Pipeline($config['processors']) : null);
        return (new Generator($config['logger']))->setVersion($config['version'])->setAliases($config['aliases'])->setNamespaces($config['namespaces'])->setAnalyser($config['analyser'])->setProcessorPipeline($processorPipeline)->setConfig($config['config'])->generate($sources, $config['analysis'], $config['validate']);
    }
    /**
    ...
    {
        $rootContext = new Context(['version' => $this->getVersion(), 'logger' => $this->getLogger()]);
     ...
     */
    public function generate(iterable $sources, ?Analysis $analysis = null, bool $validate = \true) : ?OA\OpenApi
    {
        $rootContext = new Context(['version' => $this->getVersion(), 'logger' => $this->getLogger()]);
