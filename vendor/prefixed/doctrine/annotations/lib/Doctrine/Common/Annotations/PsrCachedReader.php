...
final class PsrCachedReader implements Reader
{
    ...
    private function getLastModification(ReflectionClass $class) : int
    {
        ...
        $lastModification = max(array_merge([$filename !== \false && is_file($filename) ? filemtime($filename) : 0], array_map(function (ReflectionClass $reflectionTrait) : int {
    ...
    private function getTraitLastModificationTime(ReflectionClass $reflectionTrait) : int
    {
        ...
        $lastModificationTime = max(array_merge([$fileName !== \false && is_file($fileName) ? filemtime($fileName) : 0], array_map(function (ReflectionClass $reflectionTrait) : int {
            return $this->getTraitLastModificationTime($reflectionTrait);
